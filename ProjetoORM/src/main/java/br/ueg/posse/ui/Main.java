package br.ueg.posse.ui;

import java.util.List;
import java.util.Scanner;

import br.ueg.posse.model.Pessoa;
import br.ueg.posse.services.impl.PessoaService;
	...
	private static void listarPessoas() {
		...
		try {
			...
			pessoas.forEach(pessoa ->{
				System.out.println(String.format(" - (%d) %s %s - %d anos", pessoa.getId(), pessoa.getNome(), pessoa.getSobrenome(), pessoa.getIdade()));
			});
			if (pessoas.isEmpty()) {
